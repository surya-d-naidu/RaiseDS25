                        ...
                        membersByCategory.patron.map((member) => (
                          <div key={member.id} className="bg-white rounded-lg shadow-md p-6 border border-gray-200 transition-all hover:shadow-lg">
                            <div className="flex flex-col items-center">
                              <div className="relative mb-4">
                                {member.image ? (
                                  <div className="h-32 w-32 rounded-full overflow-hidden border-2 border-amber-200 shadow-md">
                                    <img 
                                      src={member.image} 
                                      alt={member.name} 
                                      className="h-full w-full object-cover"
                                      onError={(e) => {
                                        e.currentTarget.style.display = 'none';
                                        const avatarEl = document.getElementById(`patron-avatar-${member.id}`);
                                        if (avatarEl) {
                                          avatarEl.style.display = 'flex';
                                        }
                                      }}
                                  ...
                                  <Avatar className="h-32 w-32 bg-gradient-to-br from-amber-50 to-amber-100 text-amber-700" id={`patron-avatar-${member.id}`}>
                                    <AvatarFallback className="text-2xl font-semibold">{getInitials(member.name)}</AvatarFallback>
                                  </Avatar>
                                )}
                                {member.profileLink && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {membersByCategory.advisory_committee.length > 0 ? (
                        membersByCategory.advisory_committee.map((member) => (
                          <div key={member.id} className="bg-white rounded-lg shadow-sm p-5 border border-gray-200 hover:shadow-md transition-all group hover:border-blue-100">
                            <div className="flex items-center">
                              <div className="relative">
                                {member.image ? (
                                  <div className="h-16 w-16 rounded-full overflow-hidden border-2 border-blue-50 shadow-sm">
                                    <img 
                                      ...
                                      onError={(e) => {
                                        e.currentTarget.style.display = 'none';
                                        const avatarEl = document.getElementById(`advisory-avatar-${member.id}`);
                                        if (avatarEl) {
                                          avatarEl.style.display = 'flex';
                                        }
                                      }}
                                  ...
                                  <Avatar className="h-16 w-16 bg-gradient-to-br from-blue-50 to-blue-100 text-blue-700" id={`advisory-avatar-${member.id}`}>
                                    <AvatarFallback className="text-lg font-medium">{getInitials(member.name)}</AvatarFallback>
                                  </Avatar>
                                )}
                                {member.profileLink && (
                                  <span className="absolute -bottom-1 -right-1 bg-blue-600 text-white rounded-full p-1 shadow-sm hover:bg-blue-700 transition-colors opacity-90 group-hover:opacity-100">
                                    <a href={member.profileLink} target="_blank" rel="noopener noreferrer" aria-label="View profile">
                                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                        <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path>
                                        <polyline points="15 3 21 3 21 9"></polyline>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {membersByCategory.isps_executive.length > 0 ? (
                        membersByCategory.isps_executive.map((member) => (
                          <div key={member.id} className="bg-white rounded-lg shadow-sm p-5 border border-gray-200 hover:shadow-md transition-all group hover:border-purple-100">
                            <div className="flex items-center">
                              <div className="relative">
                                {member.image ? (
                                  <div className="h-16 w-16 rounded-full overflow-hidden border-2 border-purple-50 shadow-sm">
                                    <img 
                                      src={member.image} 
                                      alt={member.name} 
                                      className="h-full w-full object-cover"
                                  ...
                                  <Avatar className="h-16 w-16 bg-gradient-to-br from-purple-50 to-purple-100 text-purple-700" id={`isps-avatar-${member.id}`}>
                                    <AvatarFallback className="text-lg font-medium">{getInitials(member.name)}</AvatarFallback>
                                  </Avatar>
                                )}
                                {member.profileLink && (
                                  <span className="absolute -bottom-1 -right-1 bg-purple-600 text-white rounded-full p-1 shadow-sm hover:bg-purple-700 transition-colors opacity-90 group-hover:opacity-100">
                                    <a href={member.profileLink} target="_blank" rel="noopener noreferrer" aria-label="View profile">
                                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                        <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path>
                                        <polyline points="15 3 21 3 21 9"></polyline>
                            ...
                            {(member.email || member.phone) && (
                              <div className="mt-3 text-sm border-t border-gray-100 pt-3">
                                {member.email && (
                                  <a href={`mailto:${member.email}`} className="flex items-center gap-2 text-purple-600 hover:underline">
                                    <svg className="h-4 w-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                      <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path>
                                      <polyline points="22,6 12,13 2,6"></polyline>
                                    </svg>
                        ...
                        <div className="col-span-3 text-center py-12 text-gray-500">
                          <p>No ISPS executive members to display</p>
                        </div>
                      )}
                    </div>
                  </CardContent>
